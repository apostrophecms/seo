...
class JsonLdSchemaHandler {
  ...
  generateSchemas(data) {
    ...
    if (debug) {
    if (!document) {
      console.log('ℹ️  No document in this request (global-only data)');
      console.log('   This is normal for certain requests');
    } else {
      console.log('Document info:');
      console.log('  - Type:', document.type);
      console.log('  - Title:', document.title);
      console.log('  - Schema Type:', document.seoJsonLdType || '(none set)');
      const seoKeys = Object.keys(document).filter(k => k.startsWith('seo'));
      console.log('  - SEO fields:', seoKeys.length > 0 ? seoKeys.join(', ') : '(none)');
    }
  }
    const schemas = [];
