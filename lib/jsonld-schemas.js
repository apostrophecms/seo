...
class JsonLdSchemaHandler {
  ...
  getAuthorName(schemaData, document, req) {
    let authorName = null;
    ...
    // 4. Fall back to current user if available
    if (schemaData?.author?.trim()) {
      ...
      authorName = document._author[0].title || document._author[0].username;
      fallbackSource = 'document._author[0]';
    } else if (req?.user) {
      authorName = req.user.title || req.user.username;
      fallbackSource = 'req.user';
    }

    if (process.env.APOS_SEO_DEBUG && authorName && fallbackSource) {
      console.log(`[SEO] Author fallback used: ${fallbackSource} = "${authorName}"`);
    }
  ...
  getImageDataWithFallbacks(schemaData, document) {
    ...
    // 3. Fallback to simple featuredImage object with url
    ...
    // 4. Fallback to simple image object with url
    ...
    if (process.env.APOS_SEO_DEBUG && imageData && fallbackSource) {
      console.log(`[SEO] Image fallback used: ${fallbackSource}`);
    }
  ...
  getDescriptionWithFallbacks(schemaData, document) {
    ...
    if (process.env.APOS_SEO_DEBUG && description && fallbackSource !== 'schema.description') {
      console.log(`[SEO] Description fallback used: ${fallbackSource}`);
    }
  ...
  getDateWithFallbacks(schemaData, document, dateType = 'published') {
    ...
    if (process.env.APOS_SEO_DEBUG && date && fallbackSource && !fallbackSource.startsWith('schema.')) {
      console.log(`[SEO] Date (${dateType}) fallback used: ${fallbackSource}`);
    }
  ...
  getProductSchema(data) {
    ...
    } else if (process.env.APOS_SEO_DEBUG) {
      console.warn('[SEO] Product schema missing recommended image field:', document._url || document.slug);
    }
  ...
  getBreadcrumbListSchema(data) {
    ...
    return {
      '@type': 'BreadcrumbList',
      '@id': baseUrl ? `${page._url}#breadcrumb` : undefined,
