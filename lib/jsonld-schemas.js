...
class JsonLdSchemaHandler {
  ...
  generateSchemas(data) {
    ...
    const debug = process.env.APOS_SEO_DEBUG === '1' || process.env.APOS_SEO_DEBUG === 'true';
    if (debug) {
      console.log('=== JSON-LD Debug ===');
      console.log('this.getOrganizationSchema type:', typeof this.getOrganizationSchema);
    ...
    // Additional debugging for the document and its seoJsonLdType
    if (debug) {
      console.log('Document type:', document?.type);
      console.log('Document title:', document?.title);
      console.log('Document seoJsonLdType:', document?.seoJsonLdType);
      console.log('Document keys:', document ? Object.keys(document) : 'no document');
      console.log('Document SEO keys:', document ? Object.keys(document).filter(k => k.startsWith('seo')) : 'none');
    }
    const schemas = [];

    // Always include WebSite schema if we have site data
    if ((global?.seoSiteName || global?.title)) {
      if (debug) console.log('Attempting WebSite schema...');
      const websiteSchema = this.getWebsiteSchema(data);
      if (websiteSchema) {
        schemas.push(websiteSchema);
        if (debug) console.log('✓ Added WebSite schema');
      }
    }

    // Add Organization schema if configured with actual data
    if (global?.seoJsonLdOrganization?.name) {
      if (debug) console.log('Attempting Organization schema...');
      if (debug) console.log('Organization method type:', typeof this.getOrganizationSchema);
      const orgSchema = this.getOrganizationSchema(data);
      if (orgSchema) {
        schemas.push(orgSchema);
        if (debug) console.log('✓ Added Organization schema');
      }
    }

    // Add document-specific schema based on type
    if (document?.seoJsonLdType) {
      if (debug) console.log('Looking for schema generator for:', document.seoJsonLdType);
      const schemaGenerator = this.schemas[document.seoJsonLdType];
      if (schemaGenerator) {
        if (debug) console.log('✓ Found schema generator');
        const documentSchema = schemaGenerator.call(this, data);
        if (documentSchema) {
          schemas.push(documentSchema);
          if (debug) console.log('✓ Added', document.seoJsonLdType, 'schema');
        } else {
          if (debug) console.log('✗ Schema generator returned null for', document.seoJsonLdType);
        }
      } else {
        if (debug) console.log('✗ No schema generator found for:', document.seoJsonLdType);
      }
    } else {
      if (debug) console.log('✗ No seoJsonLdType set on document');
    }
    ...
    if (debug) console.log('Total schemas generated:', schemas.length);
  ...
  getOrganizationSchema(data) {
    ...
    if (org._logo?.length && org._logo[0]._urls) {
      schema.logo = {
        '@type': 'ImageObject',
        'url': org._logo[0]._urls.original || org._logo[0]._urls.full
      };
    }
  ...
  getArticleSchema(data) {
    ...
    if (document._featuredImage?.length && document._featuredImage[0]._urls) {
      const img = document._featuredImage[0];
      schema.image = img._urls.original || img._urls.full;
    }
  ...
  getProductSchema(data) {
    ...
    if (document._featuredImage?.length && document._featuredImage[0]._urls) {
      schema.image = document._featuredImage[0]._urls.original;
    }
  ...
  getLocalBusinessSchema(data) {
    ...
    const baseUrl = this.getBaseUrl(data);
    if (process.env.APOS_SEO_DEBUG === '1' || process.env.APOS_SEO_DEBUG === 'true') {
      console.log('baseUrl', baseUrl); // debug only
    }
    if (document?._url) {
      schema.url = document._url;
    } else if (baseUrl) {
  ...
  getVideoSchema(data) {
    ...
    // Thumbnail
    if (video._thumbnail?.[0]?._urls) {
      schema.thumbnailUrl = video._thumbnail[0]._urls.original || video._thumbnail[0]._urls.full;
    } else if (document._featuredImage?.[0]?._urls) {
      schema.thumbnailUrl = document._featuredImage[0]._urls.original || document._featuredImage[0]._urls.full;
    }
